...
export async function translateExamImage(imageUrl: string) {
  ...
  try {
    ...
    if(response.output_text?.includes("Not a past question")) {
      return ResponseHandler("failed", "The uploaded image is not a past question");
    }

    return ResponseHandler("success", "Image translated successfully", response.output_text || "No response");
  } catch (error: any) {
    logger.error(error, "Error in translateExamImage");
    return ResponseHandler("failed", error.message || "Error translating image");
  }
}