...
const CreateQuestionsPage = () => {
  ...
  const [mode, setMode] = useState<"new" | "update">(savedProgress && savedProgress.exam.id ? "update" : "new");
  const [existingExamsInput, setExistingExamsInput] = useState("");
  const { execute: fetchExams, status: fetchExamsStatus, res: examsRes } = useAction(getExams)
  ...
  const onSubmit = async (data: any) => {
    ...
    AppSwal.fire({
      ...
      confirmButtonText: getValues("id")
        ? "Yes, add them!"
        : "Yes, create it!",
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <Controller
                      ...
                      render={({ field }) => {
                        return (
                          <Combobox value={field.value} onChange={(exam: any) => {
                            if(!exam) return;
