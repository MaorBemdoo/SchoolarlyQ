      ...
      <motion.div initial={{y: 30, opacity: 0}} animate={{y: 0, opacity: 1}} className="grid place-content-center h-full *:z-10">
        <div className="max-w-[500px] p-4 border bg-gray-50/70 dark:bg-gray-700/70 backdrop-blur-sm rounded-md shadow-md space-y-4">
                ...
                false ? (
                  <textarea rows={6} className="resize-none rounded-md shadow-inner form-input"/>
                   ...
                   Array.from({ length: 4 }).map((_, i) => (
                       <div className="border p-2 rounded-md cursor-pointer shadow-inner bg-gray-200 hover:bg-primary-light-100 hover:border-secondary-light-400 dark:bg-inherit dark:hover:bg-primary-dark-100 disabled:bg-red-500" aria-disabled key={i}>{i + 1}</div>
                   ))
               )}
            </div>
            <div className={`collapse collapse-arrow text-white bg-green-700 ring ring-green-800 ${openExplanation ? "collapse-open" : ""}`}>
              ...
              <div className="px-4 collapse-content">Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugit qui nulla maxime dolorum. Distinctio ut, et at minus ipsa nostrum.</div>
            </div>
