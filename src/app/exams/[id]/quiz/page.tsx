...
const QuizPage = () => {
  ...
  const { res: questionRes, execute: executeGetQuestion } = useAction(getQuestion)
    ...
    <main className="w-full h-[100dvh] p-4 bg-[url(/register.jpg)] bg-cover bg-center">
      ...
      <motion.div initial={{y: 30, opacity: 0}} animate={{y: 0, opacity: 1}} className="grid place-content-center h-[calc(100%-50px)] *:z-10">
        <div className="w-full sm:w-[400px] p-4 border bg-gray-50/70 dark:bg-gray-700/70 backdrop-blur-sm rounded-md shadow-md space-y-4">
            <div className="text-center font-semibold"><span className="text-lg">{currentQuestion}</span><span className="text-2xl">/{decoded?.questionCount}</span></div>
            <div className="p-4 border bg-white rounded-md shadow-inner dark:bg-gray-800">{question?.question}</div>
            <div className="space-y-2 p-4 border bg-white rounded-md shadow-inner dark:bg-gray-800">
              {
                decoded.type == "theory" ? (
                  <textarea rows={6} className="resize-none rounded-md shadow-inner form-input"/>
               ) : (
                  question?.options ? (
                    ...
                    Array.from({ length: 4 }).map((_, i) => (
                        <div className="border p-2 rounded-md shadow-inner animate-pulse h-[40px] bg-gray-200 dark:bg-inherit" key={i} />
                    ))
                  )
               )}
            </div>
