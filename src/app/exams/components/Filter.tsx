...
const Filter = ({
  ...
  const changeHandler = (e: ChangeEvent<HTMLInputElement>) => {
    setParams((prev: any) => {
      return {
        ...prev,
        [label.toLowerCase()]: !prev[label.toLowerCase()]?.includes(
          e.target.value,
        )
          ? [...checked, e.target.value]
          : prev[label.toLowerCase()].filter(
  ...
  return (
    <div>
      ...
      {showSearch && (
        <input
          type="text"
          placeholder={`Find a ${label.slice(0, -1).toLowerCase()}`}
          ...
          .map((val, id) => (
            <li className="flex gap-2 cursor-pointer" key={id}>
              ...
              <input
                ...
                checked={checked.includes(val.toLowerCase())}
